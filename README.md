...
* **Towers**: Archer (rapid single-target), Cannon (area splash), Frost Mage (slow + damage), Cliff Sniper (long-range precision), Booster (support aura that shortens nearby towers' cooldowns and extends their range), and Farm (economic support that grants cash at the start of each wave). Each tower includes two upgrade tiers with distinct stat boosts.
...
* **Economy & Lives**: Players begin with $350 (configurable via `ReplicatedStorage.Modules.Config.GameConfig.StartingMoney`) and 30 lives. Set `ReplicatedStorage.Modules.Config.TowerConfigs.Farm.IncomePerWave` to control how much money every Farm tower grants when a new wave begins, and tweak `ReplicatedStorage.Modules.Config.TowerConfigs.Booster` to adjust its aura radius and buff multipliers per upgrade. Lives decrease when enemies reach the exit. Losing all lives ends the game for everyone; clearing every wave triggers victory.
