...
* **Economy & Lives**: Players begin with $350 (configurable via `ReplicatedStorage.Modules.Config.GameConfig.StartingMoney`) and 30 lives. Set `ReplicatedStorage.Modules.Config.GameConfig.FarmIncomePerWave` to control how much money every Farm tower grants when a new wave begins. Lives decrease when enemies reach the exit. Losing all lives ends the game for everyone; clearing every wave triggers victory.
