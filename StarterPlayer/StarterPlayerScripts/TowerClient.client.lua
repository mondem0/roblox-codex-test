...
local function createGui()
    ...
    local function updateHudLayout()
        ...
        local statusX, statusY
        if statusFrame then
            local statusSize = statusFrame.AbsoluteSize
            statusX = math.max(absoluteSize.X - statusSize.X - 20, 0)
            statusY = math.max(absoluteSize.Y - statusSize.Y - 20, 0)
            statusFrame.Position = UDim2.fromOffset(statusX, statusY)
        end

        if startButton then
            local startSize = startButton.AbsoluteSize
            local startX = math.max(absoluteSize.X - startSize.X - 20, 0)
            local baselineY = statusY or math.max(absoluteSize.Y - startSize.Y - 20, 0)
            local startY = math.max(baselineY - startSize.Y - 12, 0)
            startButton.Position = UDim2.fromOffset(startX, startY)
        end
